...
**✅ v47 CONSTRUITE (16/07)** — le moteur pense dans les 26 tâches officielles. Nouveau module `app/eco.py` : maîtrise par tâche **DÉRIVÉE du rejeu chronologique des tentatives** (zéro migration, zéro table — la leçon du noyau OJT : dériver, jamais déclarer), trois natures de rattachement pondérées (direct α · closest_fit α/2 · secondaire α/4 ; la couverture exige une tentative directe). **Readiness honnête** : tâche non couverte = 0 → l'exploit de l'audit (une zone parfaite = 41 %) donne désormais 4,1 % avec couverture 1/10 (testé). **Composeur** `/api/session/next` piloté par les tâches avec **quotas de domaine 33/41/26** et bris d'égalités aléatoire — simulation 100 apprenants neufs : 30/40/30, **25/25 tâches pourvues servies** (avant : 0 % ECO natif). Readiness **cohérent partout** : apprenant, cockpit, portrait et snapshots utilisent la même fonction (testé). Nouvel endpoint `/api/eco/mastery` (26 lignes : score, couverture, poids, fraîcheur). `conftest.py` : base de test unique pour la suite (30 tests). ⚠️ Effet visible au déploiement : le readiness BAISSE pour tout le monde (honnêteté de couverture) — à expliquer, pas à « corriger ». **Reste pour v48** : vues par tâche (mastery_list/`Me tester` reste sur les zones, cockpit heatmap, carte du portrait, pmp.js ECO_TASKS à corriger avec les intitulés officiels). ⚠️ **Échec du 1er déploiement v47 (15/07 19:46, corrigé en v47.1)** : `Exited with status 1` — le Dockerfile ne copiait que `app/`, jamais `reference/` → FileNotFoundError à l'import d'`eco.py`. Correctif : `COPY reference ./reference`. **Leçon consignée au rituel de packaging : simuler le contexte Docker** (copier uniquement ce que le Dockerfile copie, puis importer `app.main`) — les tests sur l'arbre du dépôt ne voient pas ce que l'image ne contient pas. ⏳ **v47.1 à déployer et confirmer**

**📝 Session auteur parallèle (16/07)** — prompt v2.1 adopté (`prompt-session-auteur-banque-eco-v2.1.md`) après revue par la seconde IA : rédaction depuis l'enabler vers le scénario, **codes d'erreur diagnostiques en vocabulaire fermé** (15 codes, sidecar qualité hors schéma moteur), test adversarial d'unicité des distracteurs, préflight bloquant sur compteurs recalculés (a attrapé une vraie erreur : PE8=4, pas 3), diversité de transfert (≤3 scénarios EPC/9), `calibration_status: uncalibrated_author_estimate`. Deux amendements auteur : terme banni restauré ; signal/règle de transfert cantonnés au sidecar (rationale visible = prose naturelle). Circuit : session auteur (Lot A = 9, puis STOP) → intégration ici (hashes, compteurs, tests) → audit adversarial → déploiement. **Vision consignée (Could, ne pas construire sans décision)** : boucle d'apprentissage complète (diagnostic du code d'erreur → explication ciblée → scénario de transfert → calibration de confiance) — les sidecars accumulent la matière première ; mockup d'abord le jour venu ; la calibration RÉELLE des difficultés exigera des données apprenants (taux de réussite, temps, discrimination).

**🏷️ Versionnage à quatre axes ADOPTÉ (16/07)** : application (`app-vNN`) · banque (`qbank-AAAA.MM-X`) · mapping (`mapping-2026.07.2`) · référentiel (`eco-2026-canonical-1.0`) — identifiants posés dans les fichiers de référence. Principes : le contenu se livre en **paquets de banque autonomes** (manifest + checksums), importables à tout niveau de déploiement compatible par SCHÉMA (jamais par numéro de version applicative) ; dégradation gracieuse vérifiée (sidecar ignoré par le moteur, question non mappée servie par zone et ignorée du composeur). **Le dépôt Git reste le véhicule et le registre d'audit** jusqu'à la Phase 3 (pipeline d'import à chaud côté serveur = la vraie fonctionnalité super-admin/content_scope, avec toute la discipline de validation portée serveur). Prompt auteur final : **v2.3**.

**🔓 Ouverture de test (16/07)** — décision Zoubir : plein accès pour TOUS pendant les tests, verrouillage seulement à 100 %. Interrupteur unique `OPEN_ACCESS_FOR_TESTING` dans `saas.py` (ou variable d'env `OPEN_ACCESS=0/1`) : un compte sans entitlement est résolu en `premium` → carte 26 tâches déverrouillée, séances adaptatives complètes, recommandations complètes. La matrice `free` d'origine est **préservée intacte** — re-verrouiller = basculer une seule ligne à False. Les fonctions purement institution (cockpit formateur, heatmap, séances assignées) restent réservées : ouvrir le premium ne transforme pas un apprenant en formateur. 4 tests dédiés. **Éclaircissement méthode consigné** : Claude ne voit JAMAIS l'app rendue — seulement le code du zip et les captures d'écran de Zoubir ; les captures sont ses yeux sur l'app vivante. Le cadenas 🔒 « 26 tâches » venait de `FEATURE_MATRIX['free']['full_map']='preview'` (backend), pas d'un manque de vue : **la carte 26 tâches EXISTE déjà**, v48a la branche sur le mapping v47 et fait passer la liste latérale « PRÉPARATION » aux 26 tâches.

**💡 VISION « My PMP » — compagnon PM à vie (idée Zoubir, 16/07 — à CAPTURER, ne pas construire)**
Une idée de vision, au même rang que l'OJT ou la boucle d'apprentissage : capturée, mûrie, jamais ouverte sans décision explicite. Trois couches d'un même objet, vu à trois moments de la vie du professionnel :
- **Atelier** (= l'OJT) : on apprend en faisant, sur son projet réel ; production de vrais livrables (charte, registre des risques, WBS, plan comms…).
- **Coffre** : y atterrit ce qui a passé la rubrique + validation formateur — « validé, crédible, au standard ». Porte gardée (l'IA propose, le formateur valide) ; un livrable n'entre qu'à un état de concept prouvé (Retenu/Transféré). C'est la vitrine de compétence — argument institutionnel/vente plus fort qu'un score d'examen.
- **Compagnon à vie** : le professionnel y REVIENT, des années après l'examen, quand un nouveau projet arrive — pour se structurer, retrouver ses repères, réutiliser ses livrables passés comme modèles, demander conseil. Transforme un produit qu'on achète une fois (préparer un examen) en un produit qu'on garde (outil de métier).
**Deux tensions notées** : (1) « à vie » est une promesse lourde — propriété et portabilité des livrables (les droits GDPR/18-07 jouent déjà dans ce sens), survie au-delà d'une cohorte/abonnement, modèle économique différent d'une licence qui expire ; (2) le « conseil » sur un vrai projet professionnel n'est PAS de la préparation d'examen — même prudence que l'OJT : l'outil structure et propose, ne se substitue ni au jugement du professionnel ni à une expertise qualifiée. **Question ouverte non tranchée** : « My PMP » est-il le nom de l'OJT, du coffre, ou de l'objet entier à trois couches ? À décider le jour où cette vision s'ouvrira — après le plan actuel.

**🔎 Audit systémique du plan (16/07)** — un auditeur externe a examiné le plan NON comme produit mais comme système d'exploitation/validation/commercialisation. Verdict juste : le plan est fort en histoire de développement et en vision, incomplet en machinerie institutionnelle. Deux corrections de fond appliquées : (a) **quarantaine de contenu** — seul le contenu mappé/audité est servable (garde `is_servable` dans `eco.py`, appliqué à quiz/next + composeur, testé) ; ce que j'avais appelé « dégradation gracieuse » pour une question non mappée visible était en réalité une faille de gouvernance — corrigé. (b) **Contradiction d'auth levée** : l'identité liée à session (dériver l'apprenant d'une session authentifiée, jamais d'un `learner_id` du navigateur) passe de « Could » à **Must du jalon institutionnel** — on ne peut pas faire dépendre un Must (fermeture IDOR) d'un Could. **18 angles morts capturés** dans `docs/registre-dette-angles-morts.md` avec sévérité et **déclencheur** (avant pilote réel / avant vente institutionnelle / quand l'IA touche l'OJT…) — capturés, PAS construits : la règle « une chose testable à la fois » tient. **Frontière produit adoptée comme cadrage** : Exam (entrée) → OJT (extension) → My PMP (plateforme) — ne pas bâtir trois entreprises avant d'en prouver une. **Recommandation de scinder le plan en 6 documents contrôlés** (stratégie / roadmap à portails / journal de décisions / programme de contenu / runbook ops-sécurité / registre conformité) : ADOPTÉE comme direction, à exécuter quand Zoubir le décidera — ce fichier reste la source unique d'ici là.
